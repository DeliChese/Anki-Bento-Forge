...
- [ ] **`.Codex/`**: cập nhật version trong AGENTS.md + mọi chỗ nhắc `V16.0`
...
[ ] .Codex/ skills đã cập nhật nếu cấu trúc thay đổi
```

## 5. CHANGELOG LÀ BẢN GHI SỐNG (bắt buộc)

> Quy tắc đầy đủ: [`.Codex/CHANGELOG_POLICY.md`](../../CHANGELOG_POLICY.md). Áp dụng **trong cùng thay đổi**, không đợi đến lúc đóng gói release.
...
> Nếu task của bạn làm đổi line number/signature → **CẬP NHẬT lại skill tương ứng** trong `.Codex/skills/` để giữ hệ thống luôn chính xác (chi phí thấp cho lần sau).
