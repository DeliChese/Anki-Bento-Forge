...
## `audio/engine.py` (159 dòng) — ROUTER & STATE
...
| `_selected_voice` + lock | 55-56 | ⚠️ thread-safe bắt buộc |
| `_default_speed` + lock | 59-60 | ⚠️ thread-safe bắt buộc |
| `get_voice_options(lang)` | 63 | trả `VOICE_OPTIONS.get(lang, [])` |
| `get_selected_voice(lang)` | 68 | default = giọng đầu |
| `set_selected_voice(lang, id)` | 77 | |
| `get_default_speed(lang)` | 83 | default 1.0 |
| `set_default_speed(lang, spd)` | 89 | |
| `_MODEL_LANG_MAP` | 96 | model name → lang code (ja/zh/ko, gồm cả V17 + Grammar models) |
| `detect_lang_from_model(name)` | 123 | dùng bởi reviewer hook |
| `get_audio_multilang(text, lang, voice=None, rate=None)` | 128 | **Router chính**: Edge → gTTS fallback. rate là percent string |
| `speed_to_edge_rate(speed)` | 155 | (0.25-4.0) → "-50%"..."+100%", clamp |
...
| `get_audio_voicevox(text, speaker_id=3)` | 395 | local JP, dùng `mw.col.media` |

## LUỒNG GỌI AUDIO (IMPORT)

```python
# ImportWorker (workers/import_worker.py) gọi qua _generate_audio_safe:93
# PreviewThread (workers/ai_workers.py:23) gọi `get_audio_edge_tts` trực tiếp + speed_to_edge_rate
# Reviewer hook dùng detect_lang_from_model + get_default_speed để inject speed control
get_audio_multilang(text, lang, voice, rate)   # engine.py:128 — điểm vào chính
```
