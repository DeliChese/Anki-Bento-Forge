...
1. Đặt thay đổi trong mục ngày hiện tại của `## [Unreleased]`: `### YYYY-MM-DD — Phiên bản: \`<đầu ngày>\` → \`<cuối ngày>\``. Nếu ngày đó chưa có mục, tạo mục mới ở trên cùng; không dùng tiêu đề hoặc mô tả “cập nhật đến/tới ngày”.
2. Lấy hai version từ `manifest.json` trong lịch sử Git: version của commit đầu tiên và cuối cùng trong ngày. Nếu không đổi version, ghi cùng một version ở hai vế; nếu có bump, ghi cả hai (ví dụ `17.1.0` → `17.2.0`). Đây là snapshot phát triển, không phải nhãn release.
3. Dùng một trong ba nhóm chuẩn bên trong mục ngày: `#### ✨ Added`, `#### 🔧 Changed`, `#### 🐛 Fixed`.
4. Chỉ ghi điều đã merge và có bằng chứng trong diff, test, issue đã xác nhận hoặc tài liệu kỹ thuật. Không ghi roadmap, mốc kế hoạch hay kết quả smoke/CI chưa chạy.
5. Có thể gom các thay đổi liên quan vào một bullet; không sao chép nguyên văn commit message hoặc liệt kê chi tiết implementation không giúp người dùng/maintainer hiểu tác động.
...
- Mọi thay đổi sau bản phát hành gần nhất ở `[Unreleased]`, theo mục ngày, kể cả khi `manifest.json` vẫn mang version cũ.
...
2. Kiểm tra mọi thay đổi thuộc phạm vi ở trên đều có mô tả ở đúng mục ngày trong `[Unreleased]` hoặc section version phát hành đúng; đối chiếu version đầu/cuối ngày với `git show <commit>:manifest.json`.
