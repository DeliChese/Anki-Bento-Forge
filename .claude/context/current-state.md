...
| V18.2 Contextual AI Workspaces | local implementation xanh | Reviewer sở hữu learning loop; Forge sở hữu `SOURCE → CANDIDATE → ARTIFACT`, Card Mode và đường vào Xưởng. Model history + rolling summary tách theo workspace dù transcript dùng chung; phiên cũ migrate fail-closed theo provenance. Candidate bắt buộc bám source, do người dùng chọn; current-deck match chỉ advisory. Còn GUI smoke hai surface trên profile backup và CI trước merge/release. |
...
- Bằng chứng baseline gần nhất: hai vòng `768 passed` trong isolated suite sau lát cắt Workspace-Scoped Session Memory; targeted coaching/session/workspace/reliability/candidate/i18n/docs `173 passed`. Chi tiết và điều kiện phát hành hiện hành ở [Personal Roadmap](../../work_items/PERSONAL_ROADMAP.md).
