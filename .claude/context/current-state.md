...
| Supervised AI Inventory | local implementation xanh, chờ GUI smoke | Nguồn Workshop/văn bản/Excel được quét AI theo source ID bất biến thành Giữ/Cần xem/Bỏ qua trước sản xuất; `.xlsx` giữ sheet/hàng/cột bằng parser chuẩn-library fallback, mục không neo nguyên văn hoặc thiếu quyết định fail sang Cần xem. Mặc định chỉ Giữ được sản xuất; isolated `855 passed`, còn smoke file Excel thật + API thật trên profile backup. |
