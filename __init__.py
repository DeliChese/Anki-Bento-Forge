"""
Bento Forge — Vocabulary Factory (Japanese, Chinese & Korean).

Multi-language vocabulary factory for Anki note creation, templates, and audio.
Integrated into Bento Station AIOS via bento_forge_bridge.py.
"""
...
class AnkiSmartFactory(QDialog):
    ...
    def _setup_ui(self):
        ...
        btn_reset_cost = QPushButton("↺ Reset")
        btn_reset_cost.setStyleSheet(
            "padding:3px 10px;font-size:11px;background:#34495e;color:#fff;"
            "border:none;border-radius:8px;cursor:pointer;"
        )
        btn_reset_cost.clicked.connect(self._reset_cost)
        cost_bar.addWidget(btn_reset_cost)
  ...
  "example_2_vn": "Tôi ăn cơm ngon lành."
    ...
    def _cancel_order(self):
        ...
        box.setWindowTitle("🗑️ HỦY LÔ HÀNG")
        box.setText(
            f"Lô hàng hiện có {len(self.prepared_data)} thẻ chờ lên tàu.\n\n"
    ...
    def _on_import_finished(self, report):
        ...
        msg = (
            f"🚀 XUẤT BẾN V17.0 THÀNH CÔNG! [{self._cfg()['label']}]\n"
            f"──────────────────────────────\n"
