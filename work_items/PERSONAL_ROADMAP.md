...
| --- | --- | --- |
| P0-01 — baseline test | 🟢 Local xanh | Compile toàn bộ Python và hai vòng isolated suite đều xanh `768 passed`; vẫn cần giữ gate này xanh trước merge/release. |
...
| V18.2 — Contextual AI Workspaces | 🟡 Local implementation xanh | Reviewer có learning loop kết thúc rõ ràng; Forge có `SOURCE → CANDIDATE → ARTIFACT`, manifest bám nguồn, chọn thủ công và deck-match advisory. Card Mode/artifact chỉ thuộc Forge; model history + rolling summary tách theo workspace dù transcript dùng chung. Còn GUI smoke Anki/profile restart và CI. |
...
| V18.2 | **Contextual AI Workspaces + Station UI**: Reviewer sở hữu learning loop; Forge là owner duy nhất của source production, candidate manifest, Card Mode, artifact và đường vào Xưởng; model memory tách theo workspace | P1 — local code xanh, chờ smoke | 🔴 Rất khó | `gpt-5.6-sol` / `high` | Reviewer checkpoint zero-AI/SRS; manifest Forge bám source và fail-closed; chỉ candidate chọn thủ công đi vào Card Mode; current-deck match chỉ advisory qua QueryOp; history/summary không rò hai chiều; artifact → Xưởng zero-AI; full suite + GUI smoke profile backup |
