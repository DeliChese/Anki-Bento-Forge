...
| P0-02 — smoke profile | ⏳ Chờ chủ dự án | Cần re-smoke đúng ca mục 42 + current card, xác nhận Coach không suy diễn `在 / 正在 / 正 / 呢`, các chức năng học vẫn hiện diện, transcript rộng và chỉ báo đang soạn tin hoạt động trước khi khép V18.3 hoặc release. |
...
| P2-03 — Production Drill | 🟡 Local implementation xanh | Reviewer có bài tự đặt câu zero-AI từ Usage Pattern/Collocation, gợi ý chỉ mở theo yêu cầu và không sửa SRS/collection; isolated suite `805 passed` ×2, còn GUI smoke trên profile backup. |
