...
| V18.3 — Language Study Library | 🟡 Guard sắc thái + UI local, chờ re-smoke | Scope mục số đã đúng; Coach ưu tiên tác vụ tường minh của prompt, không suy diễn sắc thái khi excerpt không đối chiếu, và card chỉ dùng khi được gọi trực tiếp hoặc hợp với ví dụ. Transcript Markdown/bảng dễ đọc, vùng chat lớn hơn, chức năng học luôn hiện diện và có chỉ báo AI đang soạn. Isolated `805 passed` ×2. Chưa khép cho tới khi smoke thật mục 42 và UI với card vocabulary/grammar đạt. |
| P1-08 — AI Deck Blueprint | 🟡 Local implementation xanh, chờ GUI smoke | Nút Deck Center trong Forge gom quản lý deck + AI Blueprint; action Tools rời đã bỏ. Blueprint nhận H1–H6, gửi source path, cho sửa cây và chỉ create/reuse sau xác nhận. Card import nhiều deck vẫn khóa tới duplicate review + undo + smoke backup. |
