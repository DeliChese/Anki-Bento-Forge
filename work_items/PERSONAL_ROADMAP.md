...
| --- | --- | --- |
| P0-01 — baseline test | 🟢 Local xanh | Isolated suite gần nhất xanh `774 passed` sau Blueprint workbench; trước đó hai vòng đều xanh `768 passed`. Vẫn cần giữ gate này xanh trước merge/release. |
...
| V18.3 — Language Study Library | ⚪ Đề xuất, chưa mở code | Study Pack thuộc profile + ngôn ngữ, dùng lại qua mọi Reviewer session cùng ngôn ngữ; Semantic Scope Resolver hiểu prompt linh hoạt, dùng catalog/chunk local và chỉ theo link nội bộ khi người học opt-in. Không chung Forge; có Card Drill nháp, không fine-tune/cloud/OCR/analytics/SRS mutation. Xem `V18.3_AI_STUDY_COACH_TRAINING_PLAN.md`. |
...
| V18.3 | **Language Study Library + Semantic Scope + Card Drill**: pack tài liệu thuộc profile + ngôn ngữ, tái dùng qua Reviewer session; prompt linh hoạt → Scope Manifest → catalog/chunk local, link nội bộ chỉ theo opt-in, giải thích/ví dụ theo source và bài tập ngắn nháp từ thẻ hiện tại | P2 — đề xuất, chưa mở | 🔴 Rất khó | `gpt-5.6-sol` / `high` | Chốt format/quota/delete và UX link opt-in cho ingest V1; chỉ pack cùng ngôn ngữ + Scope Manifest bounded vào prompt, provenance nguồn/link, tách Forge/session/cache, zero SRS/collection mutation/auto-grade, fixture paraphrase/link graph 4 ngôn ngữ + full suite + GUI smoke backup |
...
6. Chọn P2-03 Production Prompt, P2-04 Review Health hoặc V18.3 Study Coach local training theo ma sát học thực tế; Knowledge beta vẫn dormant.
