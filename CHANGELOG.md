...
- **Không nhân bản Ví dụ 3–4 khi Review** — action tái tạo chỉ gắn vào block ví dụ đã được template render; không còn tự tạo placeholder cho slot chưa thấy. Khi nạp mục từ Lịch sử AI, Factory nay xác minh ngay và đưa thẻ hợp lệ vào “Lô chờ lên tàu”, thay vì chỉ nạp JSON/tóm tắt.
...
- **Danh mục chủ đề tiền sản xuất** — mọi luồng tạo AI ngôn ngữ (từ vựng, ngữ pháp và collocation) nay lập kho trước khi tạo nội dung: đọc nhãn chủ đề có sẵn, chỉ nhờ AI bổ sung dòng thiếu, gộp các biến thể cùng tên sau khi bỏ số thứ tự/dấu câu/khác hoa-thường-dấu tiếng Việt, đếm và lưu danh mục theo checksum nguồn rồi gán lại chủ đề chuẩn cho từng ứng viên. Chỉ ứng viên **Giữ** đã có chủ đề mới được chuyển sang tạo; output ngoài kho bị loại và topic AI tự đổi bị đưa về nhãn đã khóa. Tệp lưu chỉ chứa checksum, ngôn ngữ, loại thẻ, tên chủ đề và số lượng — không chứa nội dung tài liệu. Prompt/cache contract tăng lên `40`, scanner schema tăng lên `4`.
...
- **AI preflight tách khỏi sản xuất thẻ** — quét kho chạy bằng worker riêng, chia nguồn lớn thành request nhỏ, cache theo checksum/model và không ghi Anki. Màn hình báo riêng request/input/output token thực tế của bước quét; prompt/cache version tăng lên `39` và scanner schema lên `2` để không tái dùng kết quả theo contract cũ.
