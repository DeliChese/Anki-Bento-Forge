...
#   push kết quả qua window._aiGrammarResult → hiển thị streaming + tự chấm điểm)
# ═══════════════════════════════════════════════════════════
_GRAMMAR_AI_JS = r"""
<script>
(function(){
  var GA = window._GA = window._GA || {};
  GA.state = null;
  GA.ask = function(){
    ...
    var ck = document.getElementById('ga-check');
    var rs = document.getElementById('ga-result');
    if(st) st.textContent = '⏳ AI đang sinh câu...';
    if(se) se.textContent = '';
    if(inp){ inp.value = ''; inp.style.display = 'none'; }
    if(ck) ck.style.display = 'none';
    if(rs) rs.style.display = 'none';
    try{
      var payload = btoa(unescape(encodeURIComponent(JSON.stringify({pattern:pattern,meaning:meaning,lang:lang}))));
      pycmd('ai_grammar_sentence:'+payload);
    }catch(e){}
  };
  GA.result = function(data){
    GA.state = data;
    var box = document.getElementById('ga-box');
    if(!box) return;
    var st = document.getElementById('ga-status');
    ...
    var ck = document.getElementById('ga-check');
    if(!data || data.error){
      if(st) st.textContent = '❌ ' + ((data && data.error) || 'Lỗi AI.');
      return;
    }
    var s = data.sentence || '';
    if(st) st.textContent = '✍️ Dịch câu sau sang tiếng Việt:';
    se.textContent = '';
    var i = 0;
    var t = setInterval(function(){
      i += 2;
      se.textContent = s.slice(0, i) + (i < s.length ? '▌' : '');
      if(i >= s.length){
        clearInterval(t);
        se.textContent = s;
  ...
  GA.check = function(){
    var inp = document.getElementById('ga-input');
    var rs = document.getElementById('ga-result');
    if(!inp || !rs || !GA.state) return;
    var a = (inp.value || '').trim().toLowerCase();
    var ref = (GA.state.translation || '').trim().toLowerCase();
    rs.style.display = 'block';
    var norm = function(x){ return x.replace(/[.,!?;:。！？\s]+$/, ''); };
    var ok = a && (norm(a) === norm(ref)
      || (ref && a.length >= 3 && (a.indexOf(ref.slice(0, 18)) >= 0 || ref.indexOf(a.slice(0, 18)) >= 0)));
    if(ok){ rs.className = 'combo-res combo-ok'; rs.textContent = '✅ Chính xác!'; }
    else { rs.className = 'combo-res combo-err'; rs.textContent = '❌ Gợi ý: ' + (GA.state.translation || ''); }
  };
  window._aiGrammarResult = function(data){ if(GA) GA.result(data); };
  var box = document.getElementById('ga-box');
  if(box){
    var btn = document.getElementById('ga-ask');
    if(btn) btn.addEventListener('click', GA.ask);
  }
})();
</script>
"""
