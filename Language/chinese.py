# 🇨🇳 Chinese Language Configuration

LANG_CONFIG = {
    "label": "🇨🇳 Tiếng Trung",
    "lang_code": "zh",
    "model_name": "AnkiTool Chinese V18.3 (Add-on)",
    "old_model_names": [
        "AnkiTool Chinese V18.1 (Add-on)",
        "AnkiTool Chinese V17.0 (Add-on)",
        "AnkiTool Chinese V16.0 (Add-on)",
        "AnkiTool Chinese V15.0 (Add-on)",
    ],
    "all_fields": [
        "Front", "Traditional", "Pinyin", "Meaning", "Part of Speech", "Radical Mindmap", "Usage Pattern", "Usage Note", "Collocation", "Semantic Group", "Relationship Note", "Register / Nuance", "Related Terms", "Sino-Vietnamese",
        "HSK Level", "Topic",
        "Example", "Example Fill", "Example Pinyin", "Example in Vietnamese", "Vocab Audio", "Example Audio",
        "Example2", "Example2 Fill", "Example2 Pinyin", "Example2 in Vietnamese", "Example2 Audio",
        "Example3", "Example3 Pinyin", "Example3 in Vietnamese", "Example3 Audio",
        "Example4", "Example4 Pinyin", "Example4 in Vietnamese", "Example4 Audio",
        "Example Versions", "Image", "SRS Independent",
    ],
    "json_field_map": {
        "simplified":               "Front",
        "front":                    "Front",
        "traditional":              "Traditional",
        "pinyin":                   "Pinyin",
        "meaning":                  "Meaning",
        "part_of_speech":           "Part of Speech",
        "radical_mindmap":          "Radical Mindmap",
        "usage_pattern":            "Usage Pattern",
        "usage_note":               "Usage Note",
        "collocation":              "Collocation",
        "semantic_group":           "Semantic Group",
        "relationship_note":        "Relationship Note",
        "register_nuance":          "Register / Nuance",
        "related_terms":            "Related Terms",
        "sino_vietnamese":          "Sino-Vietnamese",
        "sino-vietnamese":          "Sino-Vietnamese",
        "sinovietnamese":           "Sino-Vietnamese",
        "hsk_level":                "HSK Level",
        "hsklevel":                 "HSK Level",
        "topic":                    "Topic",
        "example":                  "Example",
        "example_pinyin":           "Example Pinyin",
        "example_vn":               "Example in Vietnamese",
        "exampleinvietnamese":      "Example in Vietnamese",
        "example_2":                "Example2",
        "example_2_pinyin":         "Example2 Pinyin",
        "example_2_vn":             "Example2 in Vietnamese",
        "example2":                 "Example2",
        "example2invietnamese":     "Example2 in Vietnamese",
        "example_3":                "Example3",
        "example_3_pinyin":         "Example3 Pinyin",
        "example_3_vn":             "Example3 in Vietnamese",
        "example_4":                "Example4",
        "example_4_pinyin":         "Example4 Pinyin",
        "example_4_vn":             "Example4 in Vietnamese",
        "image":                    "Image",
    },
    "detect_key": "simplified",
    "level_field": "HSK Level",
    "level_label": "🎓 HSK:",
    "level_choices": ["Tất cả", "HSK1", "HSK2", "HSK3", "HSK4", "HSK5", "HSK6", "HSK7-9"],
    "front_field": "Front",
    "audio_fields": [
        ("Vocab Audio", "Front"),
        ("Example Audio", "Example"),
        ("Example2 Audio", "Example2"),
        ("Example3 Audio", "Example3"),
        ("Example4 Audio", "Example4"),
    ],
    "template_names": (
        "1. Nhận diện / Combo", "2. Sản xuất", "3. Chính tả",
        "4. Phát âm", "5. Nhớ mặt chữ",
    ),
    "legacy_template_aliases": {
        "1. Tổng hợp (5 chế độ)": "1. Nhận diện / Combo",
    },
    # Dùng cho report "nghĩa khác" trong UI
    "furi_label": "Pinyin",
    "furi_json_key": "pinyin",
    "level_json_key": "hsk_level",
}


# 🇨🇳 Chinese Grammar Configuration
GRAMMAR_CONFIG = {
    "label": "🇨🇳 Ngữ pháp Tiếng Trung",
    "lang_code": "zh",
    "model_name": "AnkiTool Chinese Grammar V18.3 (Add-on)",
    "old_model_names": [
        "AnkiTool Chinese Grammar V18.1 (Add-on)",
        "AnkiTool Chinese Grammar V17.0 (Add-on)",
        "AnkiTool Chinese Grammar V16.0 (Add-on)",
    ],
    "all_fields": [
        "Pattern", "Pinyin", "Meaning", "HSK Level", "Topic",
        "Usage", "Explanation",
        "Example", "Example Pinyin", "Example in Vietnamese", "Example Audio",
        "Example2", "Example2 Pinyin", "Example2 in Vietnamese", "Example2 Audio",
        "Example3", "Example3 Pinyin", "Example3 in Vietnamese", "Example3 Audio",
        "Example4", "Example4 Pinyin", "Example4 in Vietnamese", "Example4 Audio",
        "Example Versions", "Image",
    ],
    "json_field_map": {
        "pattern":                "Pattern",
        "front":                  "Pattern",
        "pinyin":                 "Pinyin",
        "meaning":                "Meaning",
        "hsk_level":              "HSK Level",
        "hsklevel":               "HSK Level",
        "level":                  "HSK Level",
        "topic":                  "Topic",
        "usage":                  "Usage",
        "explanation":            "Explanation",
        "example":                "Example",
        "example_pinyin":         "Example Pinyin",
        "example_vn":             "Example in Vietnamese",
        "exampleinvietnamese":    "Example in Vietnamese",
        "example_2":              "Example2",
        "example_2_pinyin":       "Example2 Pinyin",
        "example_2_vn":           "Example2 in Vietnamese",
        "example2":               "Example2",
        "example2invietnamese":   "Example2 in Vietnamese",
        "example_3":              "Example3",
        "example_3_pinyin":       "Example3 Pinyin",
        "example_3_vn":           "Example3 in Vietnamese",
        "example_4":              "Example4",
        "example_4_pinyin":       "Example4 Pinyin",
        "example_4_vn":           "Example4 in Vietnamese",
        "image":                  "Image",
    },
    "detect_key": "pattern",
    "level_field": "HSK Level",
    "level_label": "🎓 HSK:",
    "level_choices": ["Tất cả", "HSK1", "HSK2", "HSK3", "HSK4", "HSK5", "HSK6", "HSK7-9"],
    "front_field": "Pattern",
    "audio_fields": [
        ("Example Audio", "Example"),
        ("Example2 Audio", "Example2"),
        ("Example3 Audio", "Example3"),
        ("Example4 Audio", "Example4"),
    ],
    "template_names": ("1. Cấu trúc → Nghĩa", "2. Nghĩa → Cấu trúc"),
    # Dùng cho report "nghĩa khác" trong UI
    "furi_label": "Pinyin",
    "furi_json_key": "pinyin",
    "level_json_key": "hsk_level",
}
