"""English target-language configuration."""


LANG_CONFIG = {
    "label": "🇬🇧 Tiếng Anh",
    "lang_code": "en",
    "model_name": "AnkiTool English V18.3 (Add-on)",
    "old_model_names": [
        "AnkiTool English V18.1 (Add-on)",
        "AnkiTool English V17.0 (Add-on)",
    ],
    "all_fields": [
        "Front", "Pronunciation", "Meaning", "Usage Pattern", "Usage Note", "Collocation", "Semantic Group", "Relationship Note", "Register / Nuance", "Related Terms", "CEFR Level", "Topic",
        "Example", "Example Fill", "Example in Vietnamese", "Vocab Audio", "Example Audio",
        "Example2", "Example2 Fill", "Example2 in Vietnamese", "Example2 Audio",
        "Example3", "Example3 in Vietnamese", "Example4", "Example4 in Vietnamese",
        "Image", "SRS Independent",
    ],
    "json_field_map": {
        "front": "Front",
        "word": "Front",
        "pronunciation": "Pronunciation",
        "ipa": "Pronunciation",
        "meaning": "Meaning",
        "usage_pattern": "Usage Pattern",
        "usage_note": "Usage Note",
        "collocation": "Collocation",
        "semantic_group": "Semantic Group",
        "relationship_note": "Relationship Note",
        "register_nuance": "Register / Nuance",
        "related_terms": "Related Terms",
        "cefr_level": "CEFR Level",
        "cefrlevel": "CEFR Level",
        "level": "CEFR Level",
        "topic": "Topic",
        "example": "Example",
        "example_vn": "Example in Vietnamese",
        "exampleinvietnamese": "Example in Vietnamese",
        "example_2": "Example2",
        "example2": "Example2",
        "example_2_vn": "Example2 in Vietnamese",
        "example2invietnamese": "Example2 in Vietnamese",
        "example_3": "Example3",
        "example_3_vn": "Example3 in Vietnamese",
        "example_4": "Example4",
        "example_4_vn": "Example4 in Vietnamese",
        "image": "Image",
    },
    "detect_key": "front",
    "level_field": "CEFR Level",
    "level_label": "🎓 CEFR:",
    "level_choices": ["Tất cả", "A1", "A2", "B1", "B2", "C1", "C2"],
    "front_field": "Front",
    "audio_fields": [
        ("Vocab Audio", "Front"),
        ("Example Audio", "Example"),
        ("Example2 Audio", "Example2"),
    ],
    "template_names": (
        "1. Nhận diện / Combo", "2. Sản xuất", "3. Chính tả",
        "4. Phát âm IPA", "5. Nhớ mặt chữ",
    ),
    "furi_label": "IPA",
    "furi_json_key": "pronunciation",
    "level_json_key": "cefr_level",
}


GRAMMAR_CONFIG = {
    "label": "🇬🇧 Ngữ pháp Tiếng Anh",
    "lang_code": "en",
    "model_name": "AnkiTool English Grammar V18.3 (Add-on)",
    "old_model_names": [
        "AnkiTool English Grammar V18.1 (Add-on)",
        "AnkiTool English Grammar V17.0 (Add-on)",
    ],
    "all_fields": [
        "Pattern", "Pronunciation", "Meaning", "CEFR Level", "Topic",
        "Usage", "Explanation",
        "Example", "Example in Vietnamese", "Example Audio",
        "Example2", "Example2 in Vietnamese", "Example2 Audio", "Image",
        "Example3", "Example3 in Vietnamese", "Example4", "Example4 in Vietnamese",
    ],
    "json_field_map": {
        "pattern": "Pattern",
        "front": "Pattern",
        "pronunciation": "Pronunciation",
        "ipa": "Pronunciation",
        "meaning": "Meaning",
        "cefr_level": "CEFR Level",
        "cefrlevel": "CEFR Level",
        "level": "CEFR Level",
        "topic": "Topic",
        "usage": "Usage",
        "explanation": "Explanation",
        "example": "Example",
        "example_vn": "Example in Vietnamese",
        "exampleinvietnamese": "Example in Vietnamese",
        "example_2": "Example2",
        "example2": "Example2",
        "example_2_vn": "Example2 in Vietnamese",
        "example2invietnamese": "Example2 in Vietnamese",
        "example_3": "Example3",
        "example_3_vn": "Example3 in Vietnamese",
        "example_4": "Example4",
        "example_4_vn": "Example4 in Vietnamese",
        "image": "Image",
    },
    "detect_key": "pattern",
    "level_field": "CEFR Level",
    "level_label": "🎓 CEFR:",
    "level_choices": ["Tất cả", "A1", "A2", "B1", "B2", "C1", "C2"],
    "front_field": "Pattern",
    "audio_fields": [("Example Audio", "Example"), ("Example2 Audio", "Example2")],
    "template_names": ("1. Cấu trúc → Nghĩa", "2. Nghĩa → Cấu trúc"),
    "furi_label": "IPA",
    "furi_json_key": "pronunciation",
    "level_json_key": "cefr_level",
}
